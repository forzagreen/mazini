...
- **Against the book.** Every one of the **16,593 printed forms** of the 495 paradigms in
...
- **Against the original.** For **7,884 inputs** (every root × pattern combination of a 154-root grid,
  plain, passive and assimilated, plus every book verb) the output is **byte-identical** to the Lua
  module's: 744,356 forms, footnotes and metadata fields, and the same 201 rejected inputs.
...
uv run ruff check
python3 tools/gen_golden.py --ar-verb DIR   # regenerate tests/fixtures/ from the module's source checkout (needs luajit)
```

`tests/fixtures/` is generated by `tools/port_fixtures.py` in the private repository that maintains the Lua
module and the book transcription:
`golden.jsonl.gz` (the module's output for 7,884 inputs), `hamza.jsonl.gz` (every hamza-seating call it
made), `loose.jsonl.gz` (the normaliser's parity set), `book/` (the 495 paradigms as CSV) and
